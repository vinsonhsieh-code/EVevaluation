...
with st.sidebar:
    ...
    # ---------- 新增：電池規格輸入（用於里程估計）----------
    st.markdown("---")
    st.subheader("🔋 電池規格 (用於里程估計)")
    battery_input_mode = st.radio("電池容量設定", ['自動估算 (根據馬達)', '手動輸入'], index=0,
                                  help="自動：根據馬達額定功率與預設運行時間估算。手動：自行輸入電池容量與電壓。")
    if battery_input_mode == '手動輸入':
        manual_battery_voltage = st.number_input("電池標稱電壓 (V)", min_value=12, value=48, step=6)
        manual_battery_capacity_ah = st.number_input("電池容量 (Ah)", min_value=1.0, value=50.0, step=5.0)
        manual_battery_energy_kwh = manual_battery_voltage * manual_battery_capacity_ah / 1000
        st.caption(f"對應能量: {manual_battery_energy_kwh:.2f} kWh")
        # 覆蓋自動估算的電池規格
        use_manual_battery = True
    else:
        use_manual_battery = False
...
# 電池估算：根據使用者選擇
if use_manual_battery:
    # 手動輸入的電池規格
    battery_spec = {
        '類型': '鋰離子電池 (Li-ion)',
        '標稱電壓 (V)': manual_battery_voltage,
        '容量 (Ah)': manual_battery_capacity_ah,
        '能量 (kWh)': manual_battery_energy_kwh,
        '放電倍率 (C)': 0,  # 未計算
        '串聯數': 0,
        '並聯數': 0,
        '估計重量 (kg)': 0
